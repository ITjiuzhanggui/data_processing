...
def upload_destructive_testing(filename):
    ...
    for row in range(1, cases.nrows):
        ...
        with db.cursor() as cursor:
            ...
            sql = f"select id from destructive_testing where pri_classification={firstId} " \
            ...
            if not query:
                ...
                sql = f"insert into destructive_testing (id, pri_classification, sec_classification, thr_classification, " \
                      f"test_version, require_content, test_start_date, test_end_date, auto_cases_num, " \
                      f"test_freq_num, test_freq_content, add_autocase_num, excu_allcase_num, blocker_bug_num, " \
                      f"critical_bug_num, major_bug_num, minor_bug_num, bugs_num, close_bug_num, open_bug_num, " \
                      f"open_bug_content, bug_description, bug_analysis, bug_resolvent, developer, tester, updater, " \
                      f"update_datetime) values(0, {firstId}, {secondId}, {thirdId}, " \
                      ...
                      f"'{cases.cell(row, 24).value}','{cases.cell(row, 25).value}','{cases.cell(row, 6).value}')"
                cursor.execute(sql)
        db.commit()
    cursor.close()
    db.close()
    print('upload of destructive test report Successfully！')


if __name__ == '__main__':
    cur_path()
    CURPATH = cur_path()
    filename = os.path.join(CURPATH, "破坏性测试数据统计.xls")
    upload_destructive_testing(filename)
