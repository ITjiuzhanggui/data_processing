...
def upload_automated_testing(filename):
    ...
    for row in range(1, cases.nrows):
        ...
        with db.cursor() as cursor:
            ...
            sql = f"select id from automated_testing where pri_classification={firstId} " \
            ...
            if not query:
                ...
                cases.cell(row, 8).value = datetime.strptime(str(int(cases.cell(row, 8).value)), '%Y%m%d')
                ...
                sql = f"insert into automated_testing (id, pri_classification, sec_classification, " \
                      f"thr_classification, test_version, require_content, intfcs_num, " \
                      f"add_intfcs_num, test_start_date, test_end_date, auto_cases_num, " \
                      f"excu_success_num, excu_fail_num, excu_allcase_num, test_freq_num, " \
                      f"test_freq_content, add_autocase_num, blocker_bug_num, critical_bug_num, " \
                      f"major_bug_num, minor_bug_num, bugs_num, close_bug_num, open_bug_num, open_bug_content," \
                      f"bug_description, bug_analysis, bug_resolvent, developer, tester, upater, " \
                      ...
                      f"'{cases.cell(row, 8).value}')"
                cursor.execute(sql)
        db.commit()
    cursor.close()
    db.close()
    print('upload of automated test report Successfully！')


if __name__ == '__main__':
    cur_path()
    CURPATH = cur_path()
    filename = os.path.join(CURPATH, "自动化测试数据统计.xls")
    upload_automated_testing(filename)
